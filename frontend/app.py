...
with st.sidebar:
    ...
    free_prefs = st.text_input(
        "Enter preferences (comma-separated)",
        placeholder="e.g. vegetarian, hiking, skip hotel, have a car"
    )
    k_value = st.slider("Number of results per category (Top K)", 1, 10, 3)
...
# --- File Upload & Itinerary Generation ---
uploaded = st.file_uploader("Choose an image", type=["png", "jpg", "jpeg"])
...
if uploaded:
    ...
    if st.button("Generate Itinerary"):
        files = {"file": (uploaded.name, uploaded.getvalue(), uploaded.type)}
        data = {
            "preferences": free_prefs,
            "top_k": k_value
        }
        with st.spinner("🧭 Processing..."):
            resp = requests.post(
                "http://localhost:8000/display-itinerary",
                files=files,
                data=data
            )
        if resp.ok:
...
# --- Display Itinerary & Chat Interface ---
if st.session_state.get('itinerary'):
    st.markdown(st.session_state['itinerary'])
    ...
    with st.form(key="chat_form"):
        user_query = st.text_input(
            "Ask a travel question (e.g., 'Best hiking trails near the hotel', 'Vegetarian restaurants in Paris')",
            key="user_query_input"
        )
        submit_chat = st.form_submit_button(label="Ask")
